...
function startPresence() {
  ...
  window.addEventListener("beforeunload", () => {
    if (me) navigator.sendBeacon?.(`https://firestore.googleapis.com/...`) ||
      updateDoc(doc(db, "users", me.uid), { online: false }).catch(() => {});
  });
}
...
async function openChat(chatId) {
  ...
  // Refresh status every 20s while open
  ...
  if (isMine) {
    ...
    delBtn.onclick = async (ev) => {
      ...
      try {
        ...
        // If it was last message, refresh lastMessage
...
function listenMessages(chatId) {
  ...
  unsubMessages = onSnapshot(q, (snap) => {
    ...
    // Keep original order (asc), then move pinned to top while preserving relative order
    const docs = [...snap.docs].sort((a, b) => {
      const ap = a.data().pinned ? 1 : 0;
      const bp = b.data().pinned ? 1 : 0;
      if (bp !== ap) return bp - ap;
      // keep original time order for same pin status
    ...
    // Notify for newest message if not from me and not current focused
    if (!snap.empty) {
      ...
      if (lastM.senderId !== me.uid && lastDoc.id !== lastNotifiedMsg[chatId]) {
        ...
        const c = conversations[chatId];
        const preview = lastM.type === "audio" ? "🎤 رسالة صوتية" :
                        lastM.type === "image" ? "📷 صورة" : (lastM.text || "رسالة جديدة");
    ...
    docs.forEach((d) => {
      ...
      if (m.type === "audio" && m.audio) {
        ...
        body = `<div class="voice-msg">
          <button class="play-btn" data-src="${m.audio}">▶</button>
          <div class="wave"></div>
      ...
      const ticks = mine
        ? `<span class="ticks ${isSeen ? "seen" : "sent"}">${isSeen ? "✓✓" : "✓"}</span>`
        : "";
      b.innerHTML = (m.pinned ? '<span class="pin-icon">📌</span>' : "") +
        body +
        `<div class="meta"><span>${fmtTime(m.createdAt)}</span>${ticks}</div>`;
      ...
      if (playBtn) {
        playBtn.onclick = (ev) => {
          ...
          playBtn.textContent = "⏸";
          audio.play().catch(() => toast("فشل التشغيل"));
      ...
      // Context menu
      let pressTimer = null;
      b.addEventListener("contextmenu", (e) => showMsgMenu(e, d.id, mine, !!m.pinned, m.text || ""));
      b.addEventListener("touchstart", (e) => {
        pressTimer = setTimeout(() => showMsgMenu(e, d.id, mine, !!m.pinned, m.text || ""), 550);
      }, { passive: true });
    ...
    if (nearBottom || !box.dataset.loaded) {
      box.scrollTop = box.scrollHeight;
      box.dataset.loaded = "1";
    }
  }, (err) => {
    console.error("messages error:", err);
    box.innerHTML = '<div class="empty-msgs">مشكلة في تحميل الرسائل</div>';
    toast("مشكلة في الرسائل");
  });
}
...
async function sendImage(file) {
  ...
  if (!file.type.startsWith("image/")) return toast("اختار صورة");
  if (file.size > 800 * 1024) return toast("الصورة كبيرة، اختار أصغر من 800KB");
  ...
  reader.onload = async () => {
    try {
      ...
      img.onload = async () => {
        ...
        await bumpConversation("📷 صورة");
        toast("تم إرسال الصورة");
      };
      img.src = reader.result;
    } catch (e) {
      console.error(e);
      toast("فشل إرسال الصورة");
    }
  };
...
async function toggleRecord() {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      if (blob.size > 400 * 1024) return toast("التسجيل طويل جدًا");
      const reader = new FileReader();
      reader.onload = async () => {
        try {
          await addDoc(collection(db, "conversations", currentChatId, "messages"), {
    ...
    $("#mic-btn").textContent = "⏹";
    toast("جاري التسجيل... اضغط تاني للإيقاف");
  } catch (e) {
    console.error(e);
...
async function startChatByIB(raw) {
  ...
  if (!other.uid) return toast("بيانات المستخدم ناقصة");
  ...
  if (!existing.exists()) {
    let otherPhoto = "";
    let otherName = other.name || "مستخدم";
    try {
    ...
    await setDoc(doc(db, "conversations", id), {
      ...
      names: { [me.uid]: me.name, [other.uid]: otherName },
  ...
  setTimeout(() => {
    if (conversations[id]) openChat(id);
    else {
      // Force open even if snapshot lag
      conversations[id] = conversations[id] || {
        id, isGroup: false, otherUid: other.uid, otherName: other.name || "مستخدم",
        otherIb: ib, otherPhoto: "", unread: 0, participants: [me.uid, other.uid],
      };
      openChat(id);
    }
  }, 400);
...
function openGroupModal() {
  ...
  box.innerHTML = "";
  const contacts = Object.values(conversations).filter((c) => !c.isGroup && c.otherUid);
  if (!contacts.length) {
    ...
    contacts.forEach((c) => {
      ...
      row.innerHTML = `<input type="checkbox" value="${c.otherUid}" data-ib="${escapeHtml(c.otherIb)}" data-name="${escapeHtml(c.otherName)}" data-photo="${c.otherPhoto || ""}" /><span>${escapeHtml(displayName(c))} <small style="color:var(--muted)">(${c.otherIb})</small></span>`;
      box.appendChild(row);
    });
  }
...
function handleProfilePhoto(file) {
  ...
  if (file.size > 500 * 1024) return toast("الصورة كبيرة، اختار أصغر من 500KB");
...
async function saveProfile() {
  ...
  try {
    ...
    // Update my name/photo in all my conversations
    const updates = Object.values(conversations).map((c) => {
      const up = {};
      up["names." + me.uid] = name;
      up["photos." + me.uid] = photoURL || "";
      return updateDoc(doc(db, "conversations", c.id), up).catch(() => {});
    });
...
async function deleteConversation() {
  ...
  const c = conversations[currentChatId];
  if (!c) return;
  if (!confirm("هل تريد حذف المحادثة دي من عندك؟ (الرسائل هتفضل موجودة)")) return;

  try {
    // Just remove me from participants or soft-delete for me
    // Simple approach: set a deleted flag for me
...
function wire() {
  ...
  $("#profile-file").onchange = (e) => {
    if (e.target.files?.[0]) handleProfilePhoto(e.target.files[0]);
  };
  ...
  $("#delete-account").onclick = async () => {
    ...
    if (!confirm("تأكيد أخير؟ كل حاجة هتتمسح")) return;
    try {
